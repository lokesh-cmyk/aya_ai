...
import { Plus, ChevronDown, ArrowUp, X, FileText, Loader2, Check, Archive, Mic } from "lucide-react";
...
const PastedContentCard: React.FC<PastedContentCardProps> = ({ content, onRemove }) => {
    return (
        <div className="relative group flex-shrink-0 w-28 h-28 rounded-2xl overflow-hidden border border-gray-300 bg-white animate-fade-in p-3 flex flex-col justify-between shadow-[0_1px_2px_rgba(0,0,0,0.05)]">
            <div className="overflow-hidden w-full">
                <p className="text-[10px] text-gray-600 leading-[1.4] font-mono break-words whitespace-pre-wrap line-clamp-4 select-none">
                    {content.content}
                </p>
            </div>

            <div className="flex items-center justify-between w-full mt-2">
                <div className="inline-flex items-center justify-center px-1.5 py-[2px] rounded border border-gray-300 bg-white">
                    <span className="text-[9px] font-bold text-gray-700 uppercase tracking-wider font-sans">PASTED</span>
                </div>
            </div>

            <button
                onClick={() => onRemove(content.id)}
                className="absolute top-2 right-2 p-[3px] bg-white border border-gray-300 rounded-full text-gray-600 hover:text-gray-900 transition-colors shadow-sm opacity-0 group-hover:opacity-100 cursor-pointer"
            >
...
const ModelSelector: React.FC<ModelSelectorProps> = ({ models, selectedModel, onSelect }) => {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            {isOpen && (
                <div className="absolute bottom-full right-0 mb-2 w-[260px] bg-white border border-gray-300 rounded-2xl shadow-2xl overflow-hidden z-50 flex flex-col p-1.5 animate-fade-in origin-bottom-right">
                    {models.map(model => (
                        <button
                            key={model.id}
                            onClick={() => {
                            ...
                            className="w-full text-left px-3 py-2.5 rounded-xl flex items-start justify-between group transition-colors hover:bg-gray-100 cursor-pointer"
                        >
...
export const ClaudeChatInput: React.FC<ClaudeChatInputProps> = ({ onSendMessage, disabled = false }) => {
    ...
    const transcribeAudio = useCallback(async (audioBlob: Blob) => {
        ...
        try {
            ...
            if (transcribedText) {
                ...
                if (originalMessage) {
                    setMessage(originalMessage + ' ' + transcribedText);
                } else {
    ...
    const startListening = useCallback(async () => {
        try {
            ...
            // Store the original message before recording starts
            originalMessageRef.current = message.trim();
    ...
    return (
        <>
            ...
            >
            <div className={`
                box-content! flex flex-col mx-2 md:mx-0 items-stretch transition-all duration-200 relative z-10 rounded-2xl cursor-text border border-gray-300
            ...
            `}>
                ...
                <div className="flex flex-col px-3 pt-3 pb-2 gap-2">
                    ...
                    <div className="relative mb-1">
                        <div className="max-h-96 w-full overflow-y-auto custom-scrollbar font-sans wrap-break-word transition-opacity duration-200 min-h-[2.5rem] pl-1">
                            <textarea
                                ref={textareaRef}
                                value={message}
                                ...
                                placeholder={disabled ? "AI is responding..." : "How can I help you today?"}
                                disabled={disabled}
                                className={`w-full bg-transparent border-0 outline-none text-gray-900 text-[16px] placeholder:text-gray-500 resize-none overflow-hidden py-0 leading-relaxed block font-normal antialiased ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
                                rows={1}
                                autoFocus
                    ...
                    <div className="flex gap-2 w-full items-center">
                        <div className="relative flex-1 flex items-center shrink min-w-0 gap-1">
                            <button
                                ...
                                className="inline-flex items-center justify-center relative shrink-0 transition-colors duration-200 h-8 w-8 rounded-lg active:scale-95 text-gray-600 hover:text-gray-900 hover:bg-gray-100 cursor-pointer"
                                type="button"
                            ...
                            <div className="flex shrink-0 min-w-8">
                                <button
                                    ...
                                    className={`transition-all duration-200 h-8 w-8 flex items-center justify-center rounded-lg active:scale-95 cursor-pointer
                                        ${isThinkingEnabled
                            ...
                            <div className="flex shrink-0 min-w-8">
                                <button
                                    ...
                                    aria-label={isListening ? "Stop recording" : isTranscribing ? "Transcribing..." : "Start voice dictation"}
                                >
                                    {isTranscribing ? (
                        ...
                        <div className="flex flex-row items-center min-w-0 gap-1">
                            ...
                            <div>
                                <button
                                    ...
                                    disabled={!canSend}
                                    className={`
                                        inline-flex items-center justify-center relative shrink-0 transition-all duration-200 h-8 w-8 rounded-xl active:scale-95
                                        ${canSend
            ...
            {isDragging && (
                <div className="absolute inset-0 bg-gray-100/90 border-2 border-dashed border-blue-600 rounded-2xl z-50 flex flex-col items-center justify-center backdrop-blur-sm pointer-events-none">
                    <Icons.Archive className="w-10 h-10 text-blue-600 mb-2 animate-bounce" />
